#!/usr/bin/env bash
#
# scripts/bootstrap.sh — one-command local setup for Kubera_EDW.
#
# Idempotent and safe to re-run. It:
#   1. installs uv (Astral) into your user account if missing — no admin/sudo,
#   2. installs a managed Python 3.12 and creates ./.venv,
#   3. installs all Python dependencies from requirements.txt,
#   4. creates .env from .env.example on first run (never overwrites an existing .env),
#   5. runs the verification gate (scripts/verify.sh).
#
# Usage:
...
# uv installs to ~/.local/bin (newer) or ~/.cargo/bin (older) — make it visible now.
...
if [ -f .env ]; then
  ...
  cp .env.example .env
  # Pin DUCKDB_PATH to an absolute path so dbt resolves it from any working directory.
